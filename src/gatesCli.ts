...
async function main() {
  try {
    ...
    lines.push("Bill Gates Public Equity Holdings (synthetic liquidation):");
    for (const r of report.perStock) {
      const status = r.exhausted ? "(INSUFFICIENT SYNTHETIC DEPTH)" : "";
      lines.push(`  ${r.symbol}:`);
      lines.push(
    ...
    lines.push(
      `Total Realized USD (synthetic): $${report.totalRealizedUsd.toLocaleString(
        undefined,
        { maximumFractionDigits: 0 }
      )}`
    );
